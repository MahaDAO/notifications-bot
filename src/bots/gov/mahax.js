...
const rest = new REST({ version: "9" }).setToken(
  `${process.env.MAHA_DiscordClientToken}`
);
  ...
  try {
    ...
    await rest.put(
      Routes.applicationGuildCommands(
        process.env.MAHA_DiscordClientId,
        process.env.MAHA_GuildId
      ),
      { body: commands }
    );
...
const mahaXBot = async () => {
  ...
  let ethToMahaPrice = Number(
    1 / JSON.parse(mahaToEthPrice)["mahadao"]["eth"]
  ).toPrecision(6);

  mahaxContract.events
    .allEvents(
      { address: `${process.env.Matic_VotingEscrow}` },
...
[MahaDAO](https://polygonscan.com/token/0xedd6ca8a4202d4a36611e2fff109648c4863ae19) | [📶 Tx Hash 📶 ](https://polygonscan.com/tx/${
        ...
        bot.sendMessage(process.env.CHAT_ID, msgTemplate, {
          parse_mode: "Markdown",
