...
const getCoinIdFromGecko = async (coinSymbol) => {
  const coinList = JSON.parse(
    await rp(`https://api.coingecko.com/api/v3/coins/list`)
  );

  const token = _.filter(
    coinList,
    (list) => list.symbol.toLowerCase() === coinSymbol.toLowerCase()
  );

  if (token.length > 1)
    throw Error("There are more than 1 token with same symbol");
...
const setTokenInfo = async () => {
  ...
  isToken0ProtocolToken =
    token0.toLowerCase() === process.env.PROTOCOL_TOKEN_ADDRESS.toLowerCase();
...
const getPriceFromGecko = async (coinId = "mahadao", currency = "usd") => {
  try {
    const priceInJsonString = await rp(
      `https://api.coingecko.com/api/v3/simple/price?ids=${coinId}&vs_currencies=usd,eth`
    );
...
🚀 **${tradeDetails.action} ${Number(
    web3.utils.fromWei(tradeDetails.mainTokenAmount, "ether")
  ).toPrecision(6)} ${tradeDetails.mainTokenSymbol}  for ${Number(
    web3.utils.fromWei(tradeDetails.secondaryTokenAmount, "ether")
  ).toPrecision(6)} ${tradeDetails.secondaryTokenSymbol}  on Uniswap


🟢  1 ${tradeDetails.mainTokenSymbol}   = ${
    tradeDetails.mainTokenPriceUSD || "-"
  }$ | ${tradeDetails.mainTokenPriceETH || "-"}ETH  🟢
🟢  1 ${tradeDetails.secondaryTokenSymbol} = ${
    tradeDetails.secondaryTokenPriceUSD || "-"
  }$ | ${tradeDetails.secondaryTokenPriceETH || "-"}ETH  🟢


📶 Tx 📶 [View](https://etherscan.io/tx/${tradeDetails.txHash})
🦄 Uniswap 🦄 [View](https://info.uniswap.org/pair/${
    process.env.UNISWAP_PAIR_ADDRESS
  })


Join $${tradeDetails.mainTokenSymbol} (${
    tradeDetails.mainTokenSymbol
  }) Telegram here (https://t.me/MahaDAO)
  `;

  bot.sendMessage(process.env.CHAT_ID, messageTemplate, {
    parse_mode: "Markdown",
  });
};
