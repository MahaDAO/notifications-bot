...
const leverage = async () => {
  leverageObj.map((lev) => {
    lev.contract.map((cont) => {
      new new Web3(lev.chainWss).eth.Contract(leverageAbi, cont.lpAdrs).events
        .allEvents()
        .on("connected", (nr) =>
        ...
        .on("data", async (data) => {
          ...
          if (
            ...
            telegramMsg = await fn.leverageTeleMsg(
              data,
              lev.chainName,
              cont.lpName
            );
            discordMsg = await fn.leverageDiscordMsg(
              data,
              lev.chainName,
              cont.lpName
            );
          ...
          bot.sendMessage(process.env.CHAT_ID, telegramMsg, {
            parse_mode: "Markdown",
